...
function buildLiveSignature(signal: LocationProfile['hazards'][number]) {
...
export function summarizeSyncResult(profiles: LocationProfile[], newAlertCount: number) {
  ...
  if (profileCount === 0) {
    return 'Add at least one coverage feed to start monitoring alerts.'
  }
  ...
  if (newAlertCount > 0) {
    parts.push(`${newAlertCount} new live alert${newAlertCount === 1 ? '' : 's'} arrived.`)
  }
