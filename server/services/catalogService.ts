...
import type { CoverageZoneSuggestion, CoverageZoneTemplate } from '../../src/types.js'
...
function matchesRegion(zone: CoverageZoneTemplate, regionCode: string | null) {
  return !regionCode || zone.regionCode === regionCode
}
...
export function listCoverageCountries() {
...
export function listCoverageRegions(countryCode: string | null) {
...
export function listCoverageZones(countryCode: string | null, regionCode: string | null) {
  return coverageCatalog
    .filter((zone) => matchesCountry(zone, countryCode))
    .filter((zone) => matchesRegion(zone, regionCode))
...
export function lookupCoverageZones(query: string, countryCode: string | null, limit = 6) {
  const normalizedQuery = normalizeText(query)

  if (!normalizedQuery) {
    return listCoverageZones(countryCode, null)
      .slice(0, limit)
